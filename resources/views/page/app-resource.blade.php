<x-layout.main-sidebar title="Application | Resource">
    ...
    <x-modals.plain-modal id="modalsAddReouce">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5">Add Resource</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form name="addResource" id="addResource" action="{{ route('do.addResource') }}"
                    method="post" autocomplete="off">
                    @csrf
                    <select class="form-select mb-3" name="sel_m_resource" id="selResource">
    ...
    <x-modals.plain-modal id="modalConnectClient">
        <div class="modal-content">
            <div class="modal-header">
                <h1 class="modal-title fs-5">Connect Client Resource</h1>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body">
                <form name="connectClient" id="connectClient" method="post" autocomplete="off">
                    @csrf
                    <select class="form-select mb-3" id="selClientNotConnected" name="sel_client">
                    </select>
                    <x-button type="submit" class="btn-outline-primary btn-sm w-100">
                        Save
                    </x-button>
        ...
        <script type="text/javascript">
            ...
            const deleteResource = (e) => {
                deleteConfirmation(
                    ...
                    'Deleting Resource Will Be Delete All Connected Client'
                );
            }
            ...
            const showConnectedApp = (e) => {
                ...
                document.getElementById('connectClientTitleTable')
                    .innerHTML = `Connected Client <u>${e.dataset.nameResource}</u>`;
            ...
            // ================ event listener =============
            document.getElementById('modalsAddReouce')
                .addEventListener('show.bs.modal', () => document.addResource.reset());
            document.getElementById('modalConnectClient')
                .addEventListener('show.bs.modal', () => document.connectClient.reset());
